...
sum{r in RelayEvents, l in Level, p in Place_Relay} (relay_points[p, l] * is_rank_p[p, r, l]) + sum{e in SoloEvents, p in Place, a in AthletesTeam} (solo_points[p]*is_ath_rank_p[p, e, a]);

# OVERALL CONSTRAINTS

# Enforce Scoring Cap
subject to Team_Scorer_Cap {t in Team}:
sum{a in AthletesTeam} scorer[a, t] = enrollment_cap;
...
sum{l in Level} athlete_swims_event_rel[a,e,l] <= scorer[a, home_team];
